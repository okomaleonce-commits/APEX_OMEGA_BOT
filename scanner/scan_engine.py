...
def run_scan(
    ...
    Returns scan_result dict with all verdicts.
    """
    t0 = time.time()
    ...
    log.info(f"Scan done: {results['scanned']} scanned | {len(results['signals'])} signals | {elapsed}s")
    return results
...
def analyse_fixture(
    ...
    """Analyse a single fixture end-to-end."""
    ...
    form_home = get_team_form(fixture["team_home_id"], last_n=5) if fixture.get("team_home_id") else []
    form_away = get_team_form(fixture["team_away_id"], last_n=5) if fixture.get("team_away_id") else []
    h2h       = get_h2h(fixture.get("team_home_id", 0), fixture.get("team_away_id", 0), last_n=5)

    if xg_home["source"] == "none" and form_home:
        xg_home = compute_goals_proxy_xg(form_home, league_cfg["home_adv"], is_home=True)
    if xg_away["source"] == "none" and form_away:
...
def analyse_by_teams(
    ...
    """Analyse a match given by team names (natural language)."""
    fixture = get_fixture_by_teams(team_home, team_away, match_date, league_id)

    if not fixture:
        log.warning(f"No fixture found: {team_home} vs {team_away} — stub mode")
        fixture = {
            "fixture_id":   0,
